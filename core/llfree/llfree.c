...
static void init_trees(llfree_t *self) {
  ...
  for (size_t tree_idx = 0; tree_idx < self->trees_len; ++tree_idx) {
    ...
    for (size_t child_idx = 0; child_idx < LLFREE_TREE_CHILDREN; ++child_idx) {
      if (child_idx >= div_ceil(self->lower.frames, LLFREE_CHILD_SIZE))
        break;
      child_t child =
          atom_load(&self->lower.children[tree_idx].entries[child_idx]);
      sum += child.free;
    }
...
llfree_meta_size_t llfree_metadata_size(size_t cores, size_t frames) {
  ...
  size_t tree_size = align_up(sizeof(tree_t) * tree_len, LLFREE_CACHE_SIZE);
  size_t local_len = LL_MIN(cores, tree_len);
  size_t local_size = align_up(sizeof(local_t), LLFREE_CACHE_SIZE) * local_len;
...
/// Searches the tree array for a tree with a free counter in the provided
...
static void swap_reserved(llfree_t *self, local_t *local, reserved_t new,
  ...
  if (old.present) {
    ...
    tree_t tree;
    atom_update(&self->trees[tree_idx], tree, tree_unreserve, free, kind);
  }
}
...
static llfree_result_t reserve_tree_and_get(llfree_t *self, size_t idx,
  ...
  tree_t old;
  if (!atom_update(&self->trees[idx], old, tree_reserve, free.min, free.max,
                   kind))
    return llfree_err(LLFREE_ERR_MEMORY);
  ...
  if (llfree_is_ok(res)) {
    ...
    reserved_t new = {.free = old.free - (treeF_t)(1 << flags.order),
    ...
    // undo reservation
    tree_t tree;
    atom_update(&self->trees[idx], tree, tree_unreserve, old.free, kind);
  }
  return res;
}
...
static llfree_result_t reserve_and_get(llfree_t *self, size_t core,
  ...
  reserve_args_t args = {.local = local, .flags = flags, .free = {0, 0}};
  ...
  res = search(self, start, 0, self->trees_len, reserve_tree_and_get, &args);
...
static bool sync_with_global(llfree_t *self, local_t *local, uint8_t kind,
  ...
  if (!atom_cmp_exchange(&local->reserved[kind], &old, new)) {
    // Revert the steal
    atom_update(&self->trees[tree_idx], old_tree, tree_inc, old_tree.free);
    return false;
  }
  return true;
}
...
static llfree_result_t get_from_local(llfree_t *self, size_t core, uint8_t kind,
  ...
  if (atom_update(&local->reserved[kind], *old, ll_reserved_dec,
    ...
    tree_t tree;
    atom_update(&self->trees[tree_from_row(old->start_row)], tree, tree_inc,
                (treeF_t)(1 << order));
  ...
  for (size_t core_off = 0; core_off < self->cores; core_off++) {
    ...
    for (uint8_t o_kind = 0; o_kind < TREE_KINDS; o_kind++) {
      ...
      if (kind < t_kind) {
        ...
        reserved_t stolen;
        res = steal_tree(self, target_core, t_kind, flags.order, &stolen);
        if (llfree_is_ok(res)) {
          local_t *local = get_local(self, core);
          swap_reserved(self, local, stolen, kind);
        }
      } else {
        // Less strict kind, just allocate
        reserved_t old;
        res = get_from_local(self, target_core, t_kind, flags.order, &old);
      }
      if (res.error != LLFREE_ERR_MEMORY)
        return res;
    }
...
static llfree_result_t alloc_global_tree(llfree_t *self, size_t idx,
                                         void *args) {
  assert(idx < self->trees_len);
...
static llfree_result_t alloc_any_global(llfree_t *self, size_t start,
  ...
  return search(self, start, 0, self->trees_len, alloc_global_tree, &args);
}
...
llfree_result_t llfree_get(llfree_t *self, size_t core, llflags_t flags) {
  ...
  if (self->trees_len > (TREE_KINDS * self->cores)) {
    for (size_t i = 0; i < RETRIES; i++) {
      res = get_from_local(self, core, kind, flags.order, &old);
      if (res.error != LLFREE_ERR_RETRY)
        break;
    }
    if (res.error != LLFREE_ERR_MEMORY && res.error != LLFREE_ERR_RETRY)
      return res;
...
llfree_result_t llfree_get_at(llfree_t *self, size_t core, uint64_t frame,
  ...
  for (size_t r = 0; r < RETRIES; r++) {
    ...
    // Decrement reserved
    if (atom_update(&local->reserved[kind], old_r, ll_reserved_dec_check,
                    tree_idx, num_frames))
      goto search_found;

    // Decrement global
    tree_t old_t;
    if (atom_update(&self->trees[tree_idx], old_t, tree_dec_force, num_frames,
                    kind))
      goto search_found;
    ...
    for (size_t o_core = 0; o_core < self->cores; o_core++) {
      ...
      for (uint8_t o_kind = 0; o_kind < TREE_KINDS; o_kind++) {
        ...
        if (atom_update(&o_local->reserved[o_kind], old, ll_steal_check,
                        tree_idx, num_frames)) {
          assert(old.present && old.free >= num_frames);

          tree_t tree;
          atom_update(&self->trees[tree_idx], tree, tree_unreserve,
                      old.free - num_frames, kind);
          goto search_found;
        }
        return llfree_err(LLFREE_ERR_MEMORY);
      }
...
llfree_result_t llfree_put(llfree_t *self, size_t core, uint64_t frame,
  ...
  bool reserve =
      LLFREE_ENABLE_FREE_RESERVE && ll_local_free_inc(local, tree_idx);
  ...
  if (reserve) {
    ...
    reserved_t new = {
        .free = free, .start_row = row_from_tree(tree_idx), .present = true};

    uint8_t kind =
        free < LLFREE_TREE_SIZE ? old_t.kind : tree_kind(self, flags);
    swap_reserved(self, local, new, kind);
  }
  return llfree_err(LLFREE_ERR_OK);
}
...
llfree_result_t llfree_reclaim(llfree_t *self, size_t core, bool hard) {
  ...
  // Search and allocate
  llfree_result_t ret = search(self, start, 0, self->trees_len, try_reclaim,
                               hard ? (void *)1 : NULL);
  if (llfree_is_ok(ret) &&
...
llfree_result_t llfree_return(llfree_t *self, uint64_t frame) {
  ...
  if (llfree_is_ok(res)) {
    ...
    atom_update(&self->trees[idx], old, tree_inc, 1 << LLFREE_HUGE_ORDER);
  }
  return res;
}
...
void llfree_print(llfree_t *self) {
  ...
  for (size_t i = 0; i < self->trees_len; i++) {
    tree_t tree = atom_load(&self->trees[i]);
    llfree_info_cont(I2 "%3" PRIuS ": free: %" PRIuS ", reserved: %d\n", i,
                     (size_t)tree.free, tree.reserved);
    if (tree.free == LLFREE_TREE_SIZE)
      free_trees += 1;
  }
  llfree_info_cont(I1 "}\n");
  ...
  for (size_t i = 0; i < self->cores; i++) {
    ...
    for (size_t kind = TREE_FIXED; kind < TREE_KINDS; kind++) {
      reserved_t ll_unused res = atom_load(&self->local[i].reserved[kind]);
      llfree_info_cont(I2 "%" PRIuS ": { present: %d, free: %" PRIu64
                          ", start: %" PRIuS "}\n",
...
#define check_equal(fmt, actual, expected)                                     \
  ({                                                                           \
    if ((actual) != (expected)) {                                              \
      llfree_info("failed: %" fmt " == %" fmt, (actual), (expected));          \
      assert(false);                                                           \
    }                                                                          \
  })

void llfree_validate(llfree_t *self) {
  check_equal(PRIuS, llfree_free_frames(self), lower_free_frames(&self->lower));
  check_equal(PRIuS, llfree_free_huge(self), lower_free_huge(&self->lower));

  for (size_t tree_idx = 0; tree_idx < self->trees_len; tree_idx++) {
    tree_t tree = atom_load(&self->trees[tree_idx]);
    check(tree.free <= LLFREE_TREE_SIZE);
    if (!tree.reserved) {
      size_t free = lower_free_at_tree(&self->lower, frame_from_tree(tree_idx));
  ...
  for (size_t core = 0; core < self->cores; core++) {
    ...
    for (size_t kind = TREE_FIXED; kind < TREE_KINDS; kind++) {
      ...
      if (res.present) {
        ...
        check(tree.free <= LLFREE_TREE_SIZE);
        size_t free =
            lower_free_at_tree(&self->lower, frame_from_tree(tree_idx));
        check_equal(PRIuS, (size_t)(tree.free + res.free), free);
      }
    }
  }
}
