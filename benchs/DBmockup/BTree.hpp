...
struct AllocGuard : public GuardX<T> {
   ...
   AllocGuard(Params&&... params) {
      GuardX<T>::ptr = reinterpret_cast<T*>(cache->allocPage());
      new (GuardX<T>::ptr) T(std::forward<Params>(params)...);
      GuardX<T>::pid = cache->toPID(GuardX<T>::ptr);
   }
};
...
struct BTree {
   ...
   void scanDesc(span<u8> key, Fn fn) {
	...
	BTree(u64 virtSize, u64 physSize, int n_threads) : splitOrdered(false) {
		if(cache==NULL){
#ifdef EXPLICIT
				cache = createMMIORegion(NULL, virtSize*gb, physSize*gb, n_threads, 64, true);
#endif
#ifdef MODULAR
