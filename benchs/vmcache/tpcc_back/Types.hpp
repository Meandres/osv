...
struct Varchar {
   ...
   Varchar(const Varchar<otherMaxLength>& other)
   {
      ...
      length = other.length;
      rte_memcpy(data, other.data, length);
   }
   ...
   Varchar<maxLength> operator||(const Varchar<otherMaxLength>& other) const
   {
      ...
      rte_memcpy(tmp.data, data, length);
      rte_memcpy(tmp.data + length, other.data, other.length);
      return tmp;
   }
...
{
   rte_memcpy(writer, x.data, x.length);
   writer[x.length] = 0;
   return x.length + 1;
}
// -------------------------------------------------------------------------------------
